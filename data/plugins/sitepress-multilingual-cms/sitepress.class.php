...
class SitePress
{
	...
	private function get_unique_post_slug( $slug, $post_id, $post_type, $post_parent, $post_language ) {
		...
		if ( $is_hierarchical ) {
			...
			foreach ( $hierarchical_post_types as $hierarchical_post_type ) {
				...
				$post_name_check_args[ 'hierarchical_post_type' ] = $hierarchical_post_type;
			}
	...
	private function is_slug_update_request() {
		$is_new_slug    = filter_input( INPUT_POST, 'new_slug', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE ) !== false;
		$is_inline_slug = filter_input( INPUT_POST, 'action' ) === 'inline-save' && filter_input( INPUT_POST, 'post_name', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE ) !== false;
	...
	function pre_get_posts( $wpq )
	{
		...
		//
		$post_action = filter_input(INPUT_POST, 'action', FILTER_SANITIZE_URL);
	...
	function filter_queries( $sql )
	{
		...
		if ( $pagenow == 'categories.php' || $pagenow == 'edit-tags.php' ) {
			if ( preg_match( '#^SELECT COUNT\(\*\) FROM ' . $wpdb->term_taxonomy . ' WHERE taxonomy = \'(category|post_tag)\' $#', $sql, $matches ) ) {
				...
				$sql          = $wpdb->prepare("
					...
					WHERE tx.taxonomy=%s AND tr.element_type=%s AND tr.language_code= %s ",
                                               $matches[1], $element_type, $current_language);
			}
		}
			...
			if ( $this->is_translated_post_type( $post_type ) ) {
				if ( preg_match( '#SELECT post_status, COUNT\( \* \) AS num_posts FROM ' . $wpdb->posts . ' WHERE post_type = \'(.+)\' GROUP BY post_status#i', $sql, $matches ) ) {
					if ( 'all' != $current_language ) {
						...
						WHERE p.post_type = %s
							AND t.element_type = %s
						GROUP BY post_status", $matches[ 1 ] , $element_type );
					}
				}
			}
		}
		...
		if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'ajax-tag-search' ) {
			...
			if ( $sql == $search ) {
				...
				$sql          = 'SELECT t.name FROM ' . $wpdb->term_taxonomy . ' AS tt
					...
					WHERE tt.taxonomy = \'' . esc_sql( $_GET[ 'tax' ] ) . '\' AND tr.language_code=\'' . esc_sql($lang) . '\' AND element_type=\'' . esc_sql($element_type) . '\'
		...
		if ( version_compare( $GLOBALS[ 'wp_version' ], '3.9', '>=' ) ) {
			if ( preg_match( "#\n\t\tSELECT ID, post_name, post_parent, post_type\n\t\tFROM {$wpdb->posts}\n\t\tWHERE post_name IN \(([^)]+)\)\n\t\tAND post_type IN \(([^)]+)\)#", $sql, $matches ) ) {
                ...
                $element_types = wpml_prepare_in( $element_types );

                $sql = $wpdb->prepare( "SELECT p.ID, p.post_name, p.post_parent, post_type
						FROM {$wpdb->posts} p
						LEFT JOIN {$wpdb->prefix}icl_translations t on t.element_id = p.ID AND t.element_type IN ({$element_types}) AND t.language_code = %s
						WHERE p.post_name IN ({$matches[1]}) AND p.post_type IN ({$matches[2]})
						ORDER BY t.language_code = %s DESC
			...
			if ( preg_match( "#SELECT ID, post_name, post_parent, post_type FROM {$wpdb->posts} WHERE post_name IN \(([^)]+)\) AND \(post_type = '([^']+)' OR post_type = 'attachment'\)#", $sql, $matches ) ) {
				$sql = $wpdb->prepare("SELECT p.ID, p.post_name, p.post_parent, post_type
						FROM {$wpdb->posts} p
						LEFT JOIN {$wpdb->prefix}icl_translations t on t.element_id = p.ID AND t.element_type = %s AND t.language_code = %s
						WHERE p.post_name IN ({$matches[1]}) AND (p.post_type = %s OR p.post_type = 'attachment')
						ORDER BY t.language_code = %s DESC
						", 'post_' . $matches[2], $current_language, $matches[2], $current_language );
				// added order by to ensure that we get the result in teh current language first
			}
		} else {
			...
			if ( preg_match( "#SELECT ID, post_name, post_parent FROM {$wpdb->posts} WHERE post_name IN \(([^)]+)\) AND \(post_type = '([^']+)' OR post_type = 'attachment'\)#", $sql, $matches ) ) {
				$sql =  $wpdb->prepare("SELECT p.ID, p.post_name, p.post_parent
						FROM {$wpdb->posts} p
						LEFT JOIN {$wpdb->prefix}icl_translations t on t.element_id = p.ID AND t.element_type = %s AND t.language_code = %s
						WHERE p.post_name IN ({$matches[1]}) AND (p.post_type = %s OR p.post_type = 'attachment')
						ORDER BY t.language_code = %s DESC
			...
			elseif ( preg_match( "#SELECT ID, post_name, post_parent FROM {$wpdb->posts} WHERE post_name = '([^']+)' AND \(post_type = '([^']+)' OR post_type = 'attachment'\)#", $sql, $matches ) ) {
				$sql = $wpdb->prepare("SELECT p.ID, p.post_name, p.post_parent
						...
						WHERE p.post_name = %s AND (p.post_type = %s OR p.post_type = 'attachment')
							AND t.language_code = %s", 'post_' . $matches[2], $matches[1], $matches[2], $current_language );
			}
		}
	...
	public static function check_settings_integrity() {
		...
		$debug_action = filter_input( INPUT_GET, 'debug_action', FILTER_SANITIZE_STRING );
		$debug_nonce  = filter_input( INPUT_GET, 'nonce', FILTER_SANITIZE_STRING );
