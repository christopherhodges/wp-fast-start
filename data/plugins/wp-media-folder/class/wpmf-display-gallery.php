...
class Wpmf_Display_Gallery {
    ...
    function wpmf_auto_insert_gallery_folder_1($gallery) {
        global $wpdb;
        $images = $wpdb->get_results( "SELECT * FROM wp_term_relationships WHERE term_taxonomy_id IN (".$gallery['folder_id'].")" );
        $img_auto = array();
    ...
    function wpmf_auto_insert_gallery_folder_0() {
        global $wpdb;
        $img_folders = $wpdb->get_results( "SELECT * FROM wp_term_relationships" );
        $img_1 = array();
        $img_a = array();
