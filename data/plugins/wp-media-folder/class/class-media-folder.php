...
class Wp_Media_Folder{
	...
	echo '<div class="error" id="wpmf_error">'
		...
		. __('You\'ve just installed WP Media Folder, to save your time we can import your media categories into WP Media Folder','wpmf')
		    . '<a href="#" class="button button-primary" style="margin: 0 5px;" onclick="importWpmfTaxonomy(true,this);" id="wmpfImportBtn">'.__('Import categories now','wpmf').' <span class="spinner"></span></a> or <a href="#" onclick="importWpmfTaxonomy(false,this);" style="margin: 0 5px;" class="button">'.__('No thanks ','wpmf').' <span class="spinner"></span></a>'
		. '</p>'
	    . '</div>';
    }
    ...
    public  function wpmf_add_image_category_filter() {
        ...
        if ( $pagenow == 'upload.php' ) {
            $dropdown_options = array( 'show_option_none'=> __( 'No Categories', 'wpmf' ) , 'option_none_value' => 0, 'hide_empty' => false, 'hierarchical' => true, 'orderby' => 'name', 'taxonomy'=>'wpmf-category', 'class'=>'wpmf-categories', 'name' => 'wcat', 'selected' => (int)(isset($_GET['wcat'])?$_GET['wcat']:0) );
	...
	<script type="text/javascript">
	    ...
	    wpmf_images_path = '<?php echo plugins_url( 'assets/images', dirname(__FILE__) ) ?>';
            ...
            //console.log(wpmf_categories, wpmf_categories_order);
	</script>
	<?php
    ...
    public function add_editor_footer() {
        wp_editor( '', 'wpmf-editor', array('media_buttons' => false,'editor_class' => 'wpmf-editor') );
    }
    ...
    public function wpmf_pre_get_posts( $query ){
       ...
       foreach ( $taxonomies as $taxonomyname => $taxonomy ) :
           if($taxonomyname == 'wpmf-category' || $taxonomyname == 'category'){
...
public function wpmf_post_results($posts){   
    ...
    public function wpmf_add_folder(){
        ...
        $updateted = wp_update_term( $inserted['term_id'], 'wpmf-category', array('term_group' => $id_author) );
        if ( is_wp_error($inserted) ) {
            // oops WP_Error obj returned, so the term existed prior
            wp_send_json($inserted->get_error_message());
        }
        $termInfos = get_term($updateted['term_id'],'wpmf-category');
    ...
    public function wpmf_delete_folder(){
        ...
        if(is_array($childs) && count($childs)>0){
            ...
            $child = get_term_children((int)$_POST['parent'],'wpmf-category');
            wp_send_json(array('status' => wp_delete_term((int)$_POST['id'],'wpmf-category'),'count_child' => count($child)));
        }
    }
    ...
    public function wpmf_move_folder(){
        ...
        if($r instanceof WP_Error){
            ...
            $child_id = get_term_children((int)$_POST['id'],'wpmf-category');
            $child_id_category = get_term_children((int)$_POST['id_category'],'wpmf-category');
            $child_parent_id = get_term_children((int)$_POST['parent_id'],'wpmf-category');
    ...
    public function wpmf_get_terms(){
        ...
        foreach ($files as $file) {
            $child = get_term_children((int)$file->term_id,'wpmf-category');
            $countchild = count($child);
            $dirs[] = array('type' => 'dir', 'dir' => $dir, 'file' => $file->name ,'id' => $file->term_id,'parent_id' => $file->parent,'count_child' => $countchild);
        }
